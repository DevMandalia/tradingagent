...
class WeeklyMVRVMACDStrategy:
    ...
    def calculate_indicators(self, df):
        ...
        df['bullish_side'] = (df['MACD'] > df['MACD_Signal']).astype(int)
        df['bearish_side'] = (df['MACD'] <= df['MACD_Signal']).astype(int)
        df['below_sma_40'] = (df['Close'] < df['SMA_40']).astype(int)
    ...
    def generate_signals(self, df):
        ...
        # Track consecutive positive MACD differences in bearish territory
        positive_count = 0
        bearish_count = 0
        ...
        for i in range(1, len(df)):
            ...
            macd_hist_diff = df['macd_histogram_diff'].iloc[i]
            ...
            # Buy conditions:
            # 1. Price below 40-week SMA and MACD difference turns positive for the second time
            # 2. On bullish side when MACD histogram difference turns positive
            # 3. MVRV ratio < 1
            if df['below_sma_40'].iloc[i] == 1:
                bearish_count += 1
                if macd_diff > 0:
                    if bearish_count == 1:
                        positive_count = 0
                    ...
                    if positive_count == 2 and mvrv_ratio < 1:
                        if self.position == 0:
                ...
                # Buy on bullish side with positive histogram difference
                if (df['bullish_side'].iloc[i] == 1 and 
                    macd_hist_diff > 0 and 
                    mvrv_ratio < 1):
                    if self.position == 0:
                        df.loc[df.index[i], 'Signal'] = 1
                        self.position = 1
            ...
            # Sell conditions:
            # 1. On bullish side and MACD difference turns negative
            # 2. MVRV ratio > 3.6
            if (df['bullish_side'].iloc[i] == 1 and 
                macd_diff < 0 and 
                mvrv_ratio > 3.6):
    ...
    def plot_results(self, df):
        ...
        # Plot equity curve
        ax2.plot(df.index, self.equity_curve, label='Weekly MVRV-MACD Strategy Equity Curve', color='purple')
        ...
        plt.tight_layout()
        plt.savefig('weekly_mvrv_macd_strategy_results.png')
        print("Plot saved as 'weekly_mvrv_macd_strategy_results.png'")
...
def main():
    ...
    strategy = WeeklyMVRVMACDStrategy(initial_capital=10000)
    weekly_df = strategy.calculate_indicators(weekly_df)
    weekly_df = strategy.generate_signals(weekly_df)
    metrics = strategy.backtest(weekly_df)
    ...
    # Plot results
    strategy.plot_results(weekly_df)
